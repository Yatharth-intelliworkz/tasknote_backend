<?php
    ...
    class TaskController extends Controller{
        ...
        public function myTaskList(Request $request){
            ...
            $companyStatuses = CompanyStatus::pluck('status', 'id');
            $users = User::all()->keyBy('id');
            
            
            // Pin Task
            
            $pinTasks = Task::where('pin', 1)->where('completed', 0)
                ->whereIn('id', $assignTask)
                ->with(['project', 'client', 'service', 'assignees', 'user'])
                ->get();
            if($ownerCheck){
                $pinChecklistItems = UserCheckList::whereIn('task_id', $pinTasks->pluck('id'))->get()->groupBy('task_id');
            } else {
                $pinChecklistItems = UserCheckList::whereIn('task_id', $pinTasks->pluck('id'))
                    ->where('user_id', $userId)
                    ->orWhere('checklist_responsible_person', $userId)
                    ->get()->groupBy('task_id');
            }
            
                
            $pinTaskDelays = TaskDelay::whereIn('task_id', $pinTasks->pluck('id'))->get()->keyBy('task_id');
            
            $pinChecklistDocuments = UserCheckList::whereIn('task_id', $pinTasks->pluck('id'))
                ->whereNotNull('file')
                ->get()->groupBy('task_id');
            
            $pinMainDocuments = Attestment::whereIn('type_id', $pinTasks->pluck('id'))
                ->where('type', 'tasks')
                ->get()->keyBy('type_id');
            ...
            foreach ($pinTasks as $task) {
                ...
                $statusName = $companyStatuses->get($task->status, 'Pending');
            
                $checkDate = Carbon::parse($task->due_date)->toDateString();
                ...
                $memberData = [];
                foreach ($task->assignees as $val) {
                    $members = array_filter(explode(",", $val->members_id));
                ...
                $typeCheckList = UserCheckList::where('task_id', $task->id)
                ...
                $typeCheckData = [];
                if (isset($pinChecklistItems[$task->id])) {
                    foreach ($pinChecklistItems[$task->id] as $val) {
                        $targetTime = $val->checklist_time_hour .':'.$val->checklist_time_minute;
                        $typeCheckData[] = [
                            'id' => $val->id,
                            'taskTypeId' => '',
                            'tasktypechecklist' => $val->checklist_name,
                            'tasktyperemark' => $val->checklist_remark,
                            'is_document' => $val->checklist_document,
                ...
                $tastDelayFlag = isset($pinTaskDelays[$task->id]) ? 1 : 0;
                
                $allCheckListCompleted = isset($pinChecklistItems[$task->id]) && $pinChecklistItems[$task->id]->where('completed', 1)->count() > 0 ? 1 : 0;
                
                $isDocument = isset($pinMainDocuments[$task->id]) || isset($pinChecklistDocuments[$task->id]) ? 1 : 0;
                $documentList = [];
                if (isset($checklistDocuments[$task->id])) {
                    foreach ($checklistDocuments[$task->id] as $doc) {
                        $documentList[] = [
                            'name' => 'document',
                            'document' => asset('public/images/userCheckList/' . $doc->file),
                        ];
                    }
                }
                ...
                $taskData = [
                    ...
                    'tastDelayFlag' => $tastDelayFlag,
                    'allCheckListCompleted' => $allCheckListCompleted,
            ...
            if($ownerCheck){
                $completedChecklistItems = UserCheckList::whereIn('task_id', $completedTasks->pluck('id'))->get()->groupBy('task_id');
            } else {
                $completedChecklistItems = UserCheckList::whereIn('task_id', $completedTasks->pluck('id'))
                    ->where('user_id', $userId)
                    ->orWhere('checklist_responsible_person', $userId)
                    ->get()->groupBy('task_id');
            }
                
            $completedTaskDelays = TaskDelay::whereIn('task_id', $completedTasks->pluck('id'))->get()->keyBy('task_id');
            
            $completedChecklistDocuments = UserCheckList::whereIn('task_id', $completedTasks->pluck('id'))
                ->whereNotNull('file')
                ->get()->groupBy('task_id');
            
            $completedMainDocuments = Attestment::whereIn('type_id', $completedTasks->pluck('id'))
                ->where('type', 'tasks')
                ->get()->keyBy('type_id');
            foreach ($completedTasks as $task) {
                ...
                $typeCheckData = [];
                if (isset($completedChecklistItems[$task->id])) {
                    foreach ($completedChecklistItems[$task->id] as $val) {
                        $targetTime = $val->checklist_time_hour .':'.$val->checklist_time_minute;
                        $typeCheckData[] = [
                            'id' => $val->id,
                            'taskTypeId' => '',
                ...
                $tastDelayFlag = isset($completedTaskDelays[$task->id]) ? 1 : 0;
                
                $allCheckListCompleted = isset($completedChecklistItems[$task->id]) && $completedChecklistItems[$task->id]->where('completed', 1)->count() > 0 ? 1 : 0;
                
                $isDocument = isset($completedMainDocuments[$task->id]) || isset($completedChecklistDocuments[$task->id]) ? 1 : 0;
                $documentList = [];
                if (isset($checklistDocuments[$task->id])) {
                    foreach ($checklistDocuments[$task->id] as $doc) {
                        $documentList[] = [
            ...
            if($ownerCheck){
                $todayChecklistItems = UserCheckList::whereIn('task_id', $todayTasks->pluck('id'))->get()->groupBy('task_id');
            } else {
                $todayChecklistItems = UserCheckList::whereIn('task_id', $todayTasks->pluck('id'))
                    ->where('user_id', $userId)
                    ->orWhere('checklist_responsible_person', $userId)
                    ->get()->groupBy('task_id');
            }
                
            $todayTaskDelays = TaskDelay::whereIn('task_id', $todayTasks->pluck('id'))->get()->keyBy('task_id');
            
            $todayChecklistDocuments = UserCheckList::whereIn('task_id', $todayTasks->pluck('id'))
                ->whereNotNull('file')
                ->get()->groupBy('task_id');
            
            $todayMainDocuments = Attestment::whereIn('type_id', $todayTasks->pluck('id'))
                ->where('type', 'tasks')
                ->get()->keyBy('type_id');
            ...
            foreach ($todayTasks as $task) {
                ...
                $typeCheckData = [];
                if (isset($todayChecklistItems[$task->id])) {
                    foreach ($todayChecklistItems[$task->id] as $val) {
                        $targetTime = $val->checklist_time_hour .':'.$val->checklist_time_minute;
                        $typeCheckData[] = [
                            'id' => $val->id,
                            'taskTypeId' => '',
                ...
                $tastDelayFlag = isset($todayTaskDelays[$task->id]) ? 1 : 0;
                
                $allCheckListCompleted = isset($todayChecklistItems[$task->id]) && $todayChecklistItems[$task->id]->where('completed', 1)->count() > 0 ? 1 : 0;
                
                $isDocument = isset($todayMainDocuments[$task->id]) || isset($todayChecklistDocuments[$task->id]) ? 1 : 0;
                $documentList = [];
                if (isset($checklistDocuments[$task->id])) {
                    foreach ($checklistDocuments[$task->id] as $doc) {
                        $documentList[] = [
            ...
            if($ownerCheck){
                $overDueChecklistItems = UserCheckList::whereIn('task_id', $overDueTasks->pluck('id'))->get()->groupBy('task_id');
            } else {
                $overDueChecklistItems = UserCheckList::whereIn('task_id', $overDueTasks->pluck('id'))
                    ->where('user_id', $userId)
                    ->orWhere('checklist_responsible_person', $userId)
                    ->get()->groupBy('task_id');
            }
                
            $overDueTaskDelays = TaskDelay::whereIn('task_id', $overDueTasks->pluck('id'))->get()->keyBy('task_id');
            
            $overDueChecklistDocuments = UserCheckList::whereIn('task_id', $overDueTasks->pluck('id'))
                ->whereNotNull('file')
                ->get()->groupBy('task_id');
            
            $overDueMainDocuments = Attestment::whereIn('type_id', $overDueTasks->pluck('id'))
                ->where('type', 'tasks')
                ->get()->keyBy('type_id');
            foreach ($overDueTasks as $task) {
                ...
                $typeCheckData = [];
                if (isset($overDueChecklistItems[$task->id])) {
                    foreach ($overDueChecklistItems[$task->id] as $val) {
                        $targetTime = $val->checklist_time_hour .':'.$val->checklist_time_minute;
                        $typeCheckData[] = [
                            'id' => $val->id,
                            'taskTypeId' => '',
                ...
                $tastDelayFlag = isset($overDueTaskDelays[$task->id]) ? 1 : 0;
                
                $allCheckListCompleted = isset($overDueChecklistItems[$task->id]) && $overDueChecklistItems[$task->id]->where('completed', 1)->count() > 0 ? 1 : 0;
                
                $isDocument = isset($overDueMainDocuments[$task->id]) || isset($overDueChecklistDocuments[$task->id]) ? 1 : 0;
                $documentList = [];
                if (isset($checklistDocuments[$task->id])) {
                    foreach ($checklistDocuments[$task->id] as $doc) {
                        $documentList[] = [
            ...
            if($ownerCheck){
                $upcomingChecklistItems = UserCheckList::whereIn('task_id', $upcomingTasks->pluck('id'))->get()->groupBy('task_id');
            } else {
                $upcomingChecklistItems = UserCheckList::whereIn('task_id', $upcomingTasks->pluck('id'))
                    ->where('user_id', $userId)
                    ->orWhere('checklist_responsible_person', $userId)
                    ->get()->groupBy('task_id');
            }
            
            $upcomingtaskDelays = TaskDelay::whereIn('task_id', $upcomingTasks->pluck('id'))->get()->keyBy('task_id');
            
            $upcomingchecklistDocuments = UserCheckList::whereIn('task_id', $upcomingTasks->pluck('id'))
                ->whereNotNull('file')
                ->get()->groupBy('task_id');
            
            $upcomingmainDocuments = Attestment::whereIn('type_id', $upcomingTasks->pluck('id'))
                ->where('type', 'tasks')
                ->get()->keyBy('type_id');
            
            // Initialize response array
            $response['upcomingTaskData'] = [];
            ...
            foreach ($upcomingTasks as $task) {
                ...
                $typeCheckData = [];
                if (isset($upcomingChecklistItems[$task->id])) {
                    foreach ($upcomingChecklistItems[$task->id] as $val) {
                        $targetTime = $val->checklist_time_hour .':'.$val->checklist_time_minute;
                        $typeCheckData[] = [
                            'id' => $val->id,
                            'taskTypeId' => '',
                ...
                $tastDelayFlag = isset($upcomingTaskDelays[$task->id]) ? 1 : 0;
                
                $allCheckListCompleted = isset($upcomingChecklistItems[$task->id]) && $upcomingChecklistItems[$task->id]->where('completed', 1)->count() > 0 ? 1 : 0;
                
                $isDocument = isset($upcomingMainDocuments[$task->id]) || isset($upcomingChecklistDocuments[$task->id]) ? 1 : 0;
                $documentList = [];
                if (isset($checklistDocuments[$task->id])) {
                    foreach ($checklistDocuments[$task->id] as $doc) {
                        $documentList[] = [
        ...
        public function taskAdd(Request $request){
            ...
            if (isset($request->is_sender)) {
                ...
                if($request->responsible_person){
                    $responsible_person = implode(',', $request->responsible_person);
                }
                 if($request->tasktype){
                    $tasktypeId = $request->tasktype;
                }
            }
            $status = 0;
            if($request->status){
                ...
                if ($request->is_recurring == 'true') {
                    if (is_array($request->client_id)) {
                        foreach ($request->client_id as $clientId) {
                            ...
                            if($request->checklistsForm){
                                $userChecklists = $request->checklistsForm;
                                $i=1;
                                foreach($userChecklists as $userChecklist){  // Loop through each checklist item
                                    $checklist = new UserCheckList();
                                    $checklist->project_id = $project_id;
                                    $checklist->task_id = $task->id;
                                    $checklist->client_id = $clientId;
                                    $checklist->task_type_id = $tasktypeId;
                                    $checklist->user_id = $userChecklist['checklist_members_id'];  // Access each item's data properly
                                    $checklist->checklist_name = $userChecklist['checklist_name'];
                                    $checklist->checklist_remark = $userChecklist['checklist_remark'];
                                    
                                    // Handle checklist_document as a boolean or string
                                    $checklist->checklist_document = $userChecklist['checklist_document'] ? 'Document Required' : 'No Document Required';
                            
                                    $checklist->checklist_time_hour = $userChecklist['checklist_time_hour'];
                                    $checklist->checklist_time_minute = $userChecklist['checklist_time_minute'];
                                    $checklist->checklist_responsible_person = $userChecklist['checklist_responsible_person'];
                                    $checklist->position = $i;
                                    $checklist->save();
                                    $i++;
                                }
                            }
                    ...
                    if (isset($request->is_sender)) {
                        if($request->checklistsForm){
                            $userChecklists = $request->checklistsForm;
                            $userCheckPhone = json_decode($userChecklists, true);
                            if (is_array($userCheckPhone)){
                                $i=1;
                                foreach($userCheckPhone as $userChecklist){  // Loop through each checklist item
                                    $checklist = new UserCheckList();
                                    $checklist->project_id = $project_id;
                                    $checklist->task_id = $task->id;
            ...
            if($taskData){
                        ...
                        if($request->members_id){
                            ...
                            foreach ($itemIds as $key => $val) {
                                ...
                                $webNotificationPayloadForMember = [
                                    'title' => 'Update Task',
                                    'body' => 'Task Has Been Updated By ' . $userName . ' For '  . $taskData->title,
                                    'icon' => 'https://msaca.tasknote.in/assets/tasknote_Favicon.svg',
                                    'url' => 'https://msaca.tasknote.in/',
                                    'status' => true,
                                    'userId' => $val,
                                    'message' => 'task update successfully',
                                ];
                ...
                if (isset($request->is_sender)) {
                    if ($request->checklistsForm) {
                        ...
                        $existingChecklists = UserCheckList::where('task_id', $taskID)->pluck('id')->toArray();
                        
                        $processedChecklistIDs = []; 
                        if (is_array($userCheckPhone)){
                            $i = 1;
                            foreach ($userCheckPhone as $key => $userChecklist) {  
                                $matchchecklist = UserCheckList::where('id', $userChecklist['id'])->where('task_id', $taskID)->first();
                                
                                if ($matchchecklist) {
                                    $matchchecklist->user_id = $userChecklist['checklist_members_id'];
                                    $matchchecklist->checklist_name = $userChecklist['checklist_name'];
                                    $matchchecklist->checklist_remark = $userChecklist['checklist_remark'];
                                    $matchchecklist->checklist_document = $userChecklist['checklist_document'] ? 'Document Required' : 'No Document Required';
                                    $matchchecklist->checklist_time_hour = $userChecklist['checklist_time_hour'];
                                    $matchchecklist->checklist_time_minute = $userChecklist['checklist_time_minute'];
                                    $matchchecklist->checklist_responsible_person = $userChecklist['checklist_responsible_person'];
                                    $matchchecklist->position = $i;
                                    $matchchecklist->update();
                                    
                                    $processedChecklistIDs[] = $matchchecklist->id;
                                } else {
                                    $newChecklist = new UserCheckList();
                                    $newChecklist->task_id = $taskID;
                                    $newChecklist->client_id = $client_id;
                                    $newChecklist->project_id = $project_id;
                                    $newChecklist->task_type_id = $tasktypeId;
                                    $newChecklist->user_id = $userChecklist['checklist_members_id'];
                                    $newChecklist->checklist_name = $userChecklist['checklist_name'];
                                    $newChecklist->checklist_remark = $userChecklist['checklist_remark'];
                                    $newChecklist->checklist_document = $userChecklist['checklist_document'] ? 'Document Required' : 'No Document Required';
                                    $newChecklist->checklist_time_hour = $userChecklist['checklist_time_hour'];
                                    $newChecklist->checklist_time_minute = $userChecklist['checklist_time_minute'];
                                    $newChecklist->checklist_responsible_person = $userChecklist['checklist_responsible_person'];
                                    $newChecklist->position = $i;
                                    $newChecklist->save();
                                    
                                    $processedChecklistIDs[] = $newChecklist->id;
                                }
                                $i++;
                            }
                        
                            UserCheckList::where('task_id', $taskID)
                                ->whereNotIn('id', $processedChecklistIDs)
                                ->delete();
                        }
                    }
                } else {
                    if ($request->checklistsForm) {
                        ...
                        // Get all existing checklist IDs for the task
                        $existingChecklists = UserCheckList::where('task_id', $taskID)->pluck('id')->toArray();
                        
                        $processedChecklistIDs = []; // Store IDs of processed checklists
                        $i = 1;
                        foreach ($userChecklists as $userChecklist) {  // Loop through each checklist item
                            $matchchecklist = UserCheckList::where('id', $userChecklist['id'])->where('task_id', $taskID)->first();
                            ...
                            if ($matchchecklist) {
                                ...
                                // Add a new checklist if it doesn't exist
                                $newChecklist = new UserCheckList();
                                $newChecklist->task_id = $taskID;
                                $newChecklist->user_id = $userChecklist['checklist_members_id'];
                                $newChecklist->checklist_name = $userChecklist['checklist_name'];
                                ...
                                // Add the new checklist ID to the processed list
                                $processedChecklistIDs[] = $newChecklist->id;
                            }
                            $i++;
                        }
                    
                        // Delete any checklist that was not processed
                        UserCheckList::where('task_id', $taskID)
                            ->whereNotIn('id', $processedChecklistIDs)
                            ->delete();
                    }
        ...
        public function fileUpload(Request $request){
            ...
            if($binaryFile){
                if ($binaryFile->getSize() > 0) {
                    ...
                    $fileNameWithoutExtension = pathinfo($binaryFile->getClientOriginalName(), PATHINFO_FILENAME);
                    $newFileName = $fileNameWithoutExtension . '_' . time() . '.' . $binaryFile->getClientOriginalExtension();
                    $path = public_path('images/all/') . $newFileName;
                    file_put_contents($path, $binaryData);
        ...
        public function taskDetail(Request $request){
            ...
            if($task){
                ...
                $allCheck = [];
                $checkList = UserCheckList::where('task_id', $task->id)->orderBy('position', 'ASC')->get();
                foreach ($checkList as $key => $check) {
                    ...
                    // }
                    $checklist_responsible_person = explode(",", $check->checklist_responsible_person);
                    $check_data = [
                            'id' => $check->id,
                            ...
                            'task_type_id' => $check->task_type_id,
                            'checklist_name' => $check->checklist_name,
                            'checklist_remark' => $check->checklist_remark,
                            'checklist_document' => $check->checklist_document,
                            'checklist_time_hour' => $check->checklist_time_hour,
                            'checklist_time_minute' => $check->checklist_time_minute,
                            'checklist_responsible_person' => User::select('id AS item_id', 'name AS item_text')->whereIn('id',$checklist_responsible_person)->get(),
       ...
       $ownerCheck = Company::where('id', $companyId)->whereRaw('FIND_IN_SET(' . $userId . ', user_id)')->first();
        ...
        public function taskDelete(Request $request){
            ...
            if($file){
                UserCheckList::where('task_id', $request->taskID)->delete();
                TaskAssigne::where('task_id', $request->taskID)->delete();
                $file->delete();
                return response()->json([
                    'status' => true,
        ...
        public function taskTypeList(Request $request){
            $companyID = $request->companyID;
            $types = TaskType::where('company_id', $companyID)->orderBy('title', 'ASC')->get();
            $typesData = [];
        ...
        public function delayTaskAdd(Request $request){
            // dd($request->all());
            $request->validate([
                'delaytask_id' => 'required',
                'reason_date' => 'required',
                'reason' => 'required',
            ]);
            $task = Task::where('id', $request->delaytask_id)->where('completed', 0)->first();;
            if($task){
                if($request->reason_date){
                    $duefromDate = trim(preg_replace('/\s*\([^)]*\)/', '', $request->reason_date));
                    $dueDateTime = new DateTime($duefromDate);
                    $dueDate = $dueDateTime->format("Y-m-d H:i:s");
                ...
                // dd($request->all());
                $delay = TaskDelay::create($delay);
                return response()->json([
                    'status' => true,
                    'message' => 'DelayTask added successfully',
                ]);
            } else {
                return response()->json([
                    'status' => false,
                    'message' => 'DelayTask added unsuccessfully',
                ]);
            }
        }
        
        public function delayTaskList(Request $request){
            $delaytask_id = $request->delaytask_id;
            $taskDelay = TaskDelay::where('task_id', $delaytask_id)->get();
            $delayData = [];
            foreach ($taskDelay as $key => $value) {
                $reason_date = date('d-m-Y',strtotime($value->reason_date));
                $pro_data = [
            ...
            if($delayData){
                return response()->json([
                    'status' => true,
                    'message' => 'Delay Tsak list successfully',
                    'data' => $delayData
                ]);
            } else {
                return response()->json([
                    'status' => false,
                    'message' => 'Delay Tsak data not found',
                    'data' => []
                ]);
            }
        }
        ...
        public function taskDocumentList(Request $request){
            $taskId = $request->taskId;
            $documentList = [];
            $taskCheckListIsDocument = UserCheckList::where('task_id', $taskId)->where('completed', '1')->whereNotNull('file')->get();
            $taskMainDocument = Attestment::where('type_id', $taskId)->where('type', 'tasks')->first();
            $mainDocument = null;
            if($taskMainDocument){
                $mainDocument = asset('public/images/all/'. $taskMainDocument->file);
            }
            if(count($taskCheckListIsDocument) > 0){
                $isDocument  = 1;
                foreach ($taskCheckListIsDocument as $key => $docs) {
                    $d_data = [
                        'name' => 'document',
                        'document' => asset('public/images/userCheckList/'. $docs->file),
                    ];
                    $documentList[] = $d_data;
                }
            }
            return response()->json([
                'status' => true,
                'message' => 'Document Tsak list successfully',
                'mainDocument' => $mainDocument,
                'data' => $documentList
            ]);
        }
        
        public function pinTaskList(Request $request){
            $request->validate([
                'companyID' => 'required',
            ]);
            $companyId = $request->companyID;
            ...
            foreach ($pinTasks as $task) {
                ...
                if($typeCheckList){
                    foreach ($typeCheckList as $key => $val) {
                        $checkcompleted = UserCheckList::where('id', $val->id)->where('task_id', $task->id)->where('completed', '1')->first();
                ...
                $allCheckListCompleted  = 0;
                $taskCheckListIsCompleted = UserCheckList::where('task_id', $task->id)->where('completed', '1')->count();
                if($taskCheckListIsCompleted > 0){
                    $allCheckListCompleted  = 1;
                }
                
                $isDocument = 0;
                $documentList = [];
                $taskCheckListIsDocument = UserCheckList::where('task_id', $task->id)->where('completed', '1')->whereNotNull('file')->get();
                if(count($taskCheckListIsDocument) > 0){
                    $isDocument  = 1;
                    foreach ($taskCheckListIsDocument as $key => $docs) {
                        $d_data = [
                ...
                $pinTaskData[] = $taskData;
                    
            }
            if($pinTaskData){
                return response()->json([
                    'status' => true,
                    'message' => 'Pin task list successfully',
                    'add' => $add,
                    'edit' => $edit,
                    'delete' => $delete,
                    'data' => $pinTaskData
                ...
                return response()->json([
                    'status' => false,
                    'message' => 'Pin task data not found',
                    'data' => []
                ]);
            }
        }
        ...
        public function completedTaskList(Request $request){
            ...
            foreach ($completedTasks as $task) {
                ...
                $completedTaskData[] = $taskData;
                    
            }
            if($completedTaskData){
                return response()->json([
                    'status' => true,
                    'message' => 'Completed task list successfully',
                    'add' => $add,
                    'edit' => $edit,
                    'delete' => $delete,
                ...
                return response()->json([
                    'status' => false,
                    'message' => 'Completed task data not found',
                    'data' => []
                ]);
            }
        }
        
        public function todayTaskList(Request $request){
            $request->validate([
                'companyID' => 'required',
            ]);
            $companyId = $request->companyID;
            ...
            foreach ($todayTasks as $task) {
                ...
                $taskMainDocument = Attestment::where('type_id', $task->id)->where('type', 'tasks')->first();
                if($taskMainDocument){
                    $isDocument = 1;
                }
            ...
            if($todayTaskData){
                return response()->json([
                    'status' => true,
                    'message' => 'Today task list successfully',
                    'add' => $add,
                    'edit' => $edit,
                    'delete' => $delete,
                    'data' => $todayTaskData
                ...
                return response()->json([
                    'status' => false,
                    'message' => 'Today task data not found',
                    'data' => []
                ]);
            }
        }
        
        public function overDueTaskList(Request $request){
            $request->validate([
                'companyID' => 'required',
            ]);
            $companyId = $request->companyID;
            ...
            foreach ($overDueTasks as $task) {
                ...
                $overDueTaskData[] = $taskData;
                    
            }
            if($overDueTaskData){
                return response()->json([
                    'status' => true,
                    'message' => 'Over Due task list successfully',
                    'add' => $add,
                    'edit' => $edit,
                    'delete' => $delete,
                ...
                return response()->json([
                    'status' => false,
                    'message' => 'Over Due task data not found',
                    'data' => []
                ]);
            }
        }
        
        public function upcomingTaskList(Request $request){
            $request->validate([
                'companyID' => 'required',
            ]);
            ...
            $userModel = $userData ? UserPermission::where('user_role_id', $userData->assignRole)->where('user_model_id', 2)->first() : null;
            ...
            // Get all task IDs for assigned tasks based on ownership or assignment criteria
            $assignTask = $ownerCheck
                ? Task::where('company_id', $companyId)->pluck('id')->toArray()
                : array_unique(array_merge(
                    TaskAssigne::whereRaw('FIND_IN_SET(' . $userId . ', members_id)')->pluck('task_id')->toArray(),
                    TaskAssigne::whereIn('team_id', Team::where('company_id', $companyId)->whereRaw('FIND_IN_SET(' . $userId . ', members_id)')->pluck('id')->toArray())->pluck('task_id')->toArray(),
                    Task::where('user_id', $userId)->where('company_id', $companyId)->pluck('id')->toArray(),
                    SubTask::whereRaw('FIND_IN_SET(' . $userId . ', assigne_id)')->pluck('task_id')->toArray()
                ));
            ...
            // Fetch all upcoming tasks with necessary relationships
            $upcomingTasks = Task::where('pin', 0)
                ->where('completed', 0)
                ->whereIn('id', $assignTask)
                ->whereDate('due_date', '>', $paymentDate)
            ...
            // Pre-fetch related data to avoid nested queries in the loop
            $checklistItems = UserCheckList::whereIn('task_id', $upcomingTasks->pluck('id'))
                ->where(function($query) use ($userId) {
                    $query->where('user_id', $userId)
                          ->orWhere('checklist_responsible_person', $userId);
                })
            ...
            $taskDelays = TaskDelay::whereIn('task_id', $upcomingTasks->pluck('id'))->get()->keyBy('task_id');
            
            $checklistDocuments = UserCheckList::whereIn('task_id', $upcomingTasks->pluck('id'))
                ->whereNotNull('file')
                ->get()->groupBy('task_id');
            
            $mainDocuments = Attestment::whereIn('type_id', $upcomingTasks->pluck('id'))
                ->where('type', 'tasks')
                ->get()->keyBy('type_id');
            ...
            foreach ($upcomingTasks as $task) {
                ...
                // Checklist data
                $typeCheckData = [];
                if (isset($checklistItems[$task->id])) {
                    foreach ($checklistItems[$task->id] as $val) {
                        if (!$val->completed) {
                            $typeCheckData[] = [
                                'id' => $val->id,
                                'taskTypeId' => '',
                ...
                // Task delay flag
                $tastDelayFlag = isset($taskDelays[$task->id]) ? 1 : 0;
            
                // Checklist completion flag
                $allCheckListCompleted = isset($checklistItems[$task->id]) && $checklistItems[$task->id]->where('completed', 1)->count() > 0 ? 1 : 0;
            
                // Document data
                $isDocument = isset($mainDocuments[$task->id]) || isset($checklistDocuments[$task->id]) ? 1 : 0;
                $documentList = [];
                if (isset($checklistDocuments[$task->id])) {
                    foreach ($checklistDocuments[$task->id] as $doc) {
                        $documentList[] = [
                ...
                $upcomingTaskData[] = $taskData;
            }
        
            if($upcomingTaskData){
                return response()->json([
                    'status' => true,
                    'message' => 'UpComing task list successfully',
                    'add' => $add,
                    'edit' => $edit,
                    'delete' => $delete,
                ...
                return response()->json([
                    'status' => false,
                    'message' => 'UpComing task data not found',
                    'data' => []
                ]);
            }
        }
        ...
        function getInnerCheckList($taskId){
            $userId = Auth::user()->id;
            $checklists = UserCheckList::where('task_id', $taskId)
                ->where(function ($query) use ($userId) {
                    $query->where('user_id', $userId)->orWhere('checklist_responsible_person', $userId);
                })
                ->where('completed', 0)
                ->get()
                ->map(function ($checklist) {
                    return [
                        'id' => $checklist->id,
                        'taskTypeId' => '',
                        'tasktypechecklist' => $checklist->checklist_name,
                        'tasktyperemark' => $checklist->checklist_remark,
                        'is_document' => $checklist->checklist_document,
                    ];
                });
        ...
        function getInnerMemberList($task){
            $memberData = $task->flatMap(function ($assignee) {
                    return User::whereIn('id', explode(",", $assignee->members_id))->get()->map(function ($user) {
                        return [
                            'id' => $user->id,
                            'name' => $user->name,
                            'is_login' => $user->is_login,
