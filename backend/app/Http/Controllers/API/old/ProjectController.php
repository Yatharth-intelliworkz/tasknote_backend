<?php
    ...
    class ProjectController extends Controller{
        ...
        public function projectGet(){
            ...
            if($project){
                ...
                $response = [
                    ...
                    'membersName' => $project->members_id,
            ...
            if($userId){
                ...
                if($project){
                    if($request->taskType){
                        ...
                        ProjectTypeList::where('project_id', $project->id)->delete();
                        foreach ($taskTypeData as $key => $val) {
                            if($val['hours']){
                                $hours = $val['hours'];
                            } else {
            ...
            if($company){
                ...
                if($projectData){
                    foreach ($projectData as $key => $PVal) {
                        $totalTaskList = Task::where('project_id', $PVal->id)->where('user_id', $userId)->get();
                        ...
                        $taskCost = 0;
                        if ($totalTaskList) {
                            foreach ($totalTaskList as $key => $value) {
                                $taskCost += intval($value->remainingtotalCost); // Add the remaining total cost of each task
                            }
                        }
                        $remainingCost = 30000;
                        // $remainingCost = $PVal->total_cost - $taskCost;
                        // dd($taskCost);
                        $pro_data = [
                            'id' => $PVal->id,
                            'name' => $PVal->name,
                ...
                foreach ($sahrenote as $key => $value) {
                    ...
                    // dd($sahrenote);
                        //                     $sNote = Note::where(function ($query) use ($value, $paymentDate) {
                        //     $query->where('id', $value->note_id)
                        //           ->orWhereDate('expiry_date', '<=', $paymentDate);
        ...
        public function dashboardUserByCount(){
            ...
            if($company){
                $partnerAllData = [];
                $partnerData = User::where('user_type',3)->get();
                if($partnerData){
                ...
                $managerAllData = [];
                $managerData = User::where('user_type',4)->get();
                if($managerData){
