<?php
    ...
    class UserController extends Controller{
        ...
        public function partnerMemberList(Request $request){
            $companyID = $request->companyID;
            $member = User::where('company_id', $companyID)->where('user_type', 3)->whereHas(
                'roles', function($q){
                    $q->where('name', 'user');
                }
            )->get();
        ...
        public function userDetails(Request $request)
{
    ...
    $taskIds = TaskAssigne::whereIn('team_id', $teamIds)
        ->orWhere('members_id', 'LIKE', "%$userID%")
        ->pluck('task_id');
    ...
    $totalCounts = [
        ...
        'totalOverdueTasks' => Task::whereIn('id', $allTaskIds)->where('due_date', '<', now())->count(),
