    ...
    class TaskController extends Controller{
        ...
        public function myTaskList(Request $request){
            ...
            foreach ($pinTasks as $task) {
                ...
                // $mainCheckList = DB::table('user_check_lists')
                // ->where('task_id', $task->id)
                ...
                // if($mainCheckList){
                //     foreach ($mainCheckList as $key => $check) {
                //         $checkListData = CheckList::where('id', $check->checklist_id)->first();
                //         if($checkListData){
                //             $c_data = [
                //                 'id' => $check->id,
                //                 'remark' => $checkListData->remark,
                //                 'ckluser_id' => $checkListData->assigne_id,
                //                 'hours' => $checkListData->hour,
                //                 'minutes' => $checkListData->minute,
                //             ];
                //             $checkData[] = $c_data;
                //         }
                //     }
    ...
    protected function getUserPermissions($userId) {
        ...
        public function taskDetail(Request $request){
            ...
            if($task){
                ...
                $response = [
                    ...
                    'type_id' => TaskType::select('id AS item_id', 'title AS item_text')->whereIn('id',$project_id)->get(),
        ...
        public function taskCheckListCompleted(Request $request){
            ...
            if($task){
                ...
                $task->completedNote = $request->completedNote;
                $task->save();
                if($completed == 1){
                    return response()->json([
                        'status' => true,
