<?php
    ...
    class UserController extends Controller{
        ...
        public function index(Request $request){
            ...
            // $test = 'cJ02BSllScq6PwdzXPIcK8:APA91bG27Vh34bGL7BX_EJWVeOgUNKbB2Yot6b0-9YGDF-9lvAaEvVWG5rSWW16T4BLvYymuUHgGcMdQdHeXGey3TOB_PEzBGzARqFMTErZ0b3CrgQkyDtdPTIrNzzNYX3cfyEEjYZN2';
            // FCMService::send(
            //     $test,
            //     [
        ...
        public function changePassword(Request $request){
            ...
            if($user){
             ...
             Mail::send('mail.passwordUpdate', ['info' => $passwordData], function ($message) use ($passwordData, $email) {
        ...
        public function partnerMemberList(Request $request){
            $companyID = $request->companyID;
            $member = User::where('company_id', $companyID)->where('user_type', 3)->orWhere('user_type', 2)->whereHas(
                'roles', function($q){
                    $q->where('name', 'user');
                }
            )->get();
            ...
            if($userId){
                ...
                if($checkEmail){
                    return response()->json([
                        'status' => false,
                        'code' => 210,
                        'message' => 'User email alrady exist',
                    ]);
                }
                ...
                if($request->hasFile('profile') && $request->file('profile')->isValid()){
                    ...
                    $member = [ 
                        ...
                        'profile'=>$imageName,
                        'created_by'=>$userId,
                        'hour_per_cost'=>$request->hour_per_cost,
                    ];
                }else {
                    $member = [
                        'company_id' => $request->companyID,
                ...
                if($member){
                    $createdByData = User::where('id',$userId)->first();
                    $subAssignInfo = [
                        'mainName' => $request->name,
                        'email' => $request->email_id,
                    ...
                    Mail::send('mail.new_member', ['info' => $subAssignInfo], function ($message) use ($subAssignInfo) {
                        $message->to($subAssignInfo['email'])->subject('Task Note | Login Credentials');
                    });
                    return response()->json([
                        'status' => true,
        ...
        public function comapnyMemberGet(Request $request){
            ...
            if($member){
                ...
                $response = [
                    ...
                    'hour_per_cost' => $member->hour_per_cost,
        ...
        public function comapnyMemberEdit(Request $request){
            ...
            if($userId){
                ...
                $dobDate = date('Y-m-d');
                if($request->dob){
                    $dob = trim(preg_replace('/\s*\([^)]*\)/', '', $request->dob));
                    $dobDateTime = new DateTime($dob);
                    $dobDate = $dobDateTime->format("Y-m-d");
                }
                $member = User::find($request->memberId);
                if($request->hasFile('profile') && $request->file('profile')->isValid()){
                    $imageName = mt_rand(10000000000,99999999999).'.'.$request->profile->extension();  
                    ...
                    $member->assignRole = $request->assignRole;
                    $member->gender = $request->gender;
                    $member->is_active = $is_active;
                    $member->user_type = $request->user_type;
                    $member->hour_per_cost = $request->hour_per_cost;
        ...
        public function userDetails(Request $request)
        {
            ...
            // $teamIds = Team::where('members_id', 'LIKE', "%$userID%")->pluck('id');
        
            $taskIds = TaskAssigne::whereRaw('FIND_IN_SET(' . $userID . ', members_id)')->pluck('task_id');
        
            $myTaskIds = Task::where('user_id', $userID)
                ->where('company_id', $companyID)
                ->pluck('id');
        
            $allTaskIds = $taskIds->merge($myTaskIds)->unique();
            // dd($allTaskIds);
            $totalCounts = [
                'totalAllTasks' => $allTaskIds->count(),
                'totalCompletedTasks' => Task::whereIn('id', $allTaskIds)->where('completed', 1)->count(),
                'totalIncompleteTasks' => Task::whereIn('id', $allTaskIds)->where('completed', 0)->count(),
                'totalOverdueTasks' => Task::whereIn('id', $allTaskIds)->where('completed', 0)->where('due_date', '<', now())->count(),
                'totalLowPriorityTasks' => Task::whereIn('id', $allTaskIds)->where('priority', 0)->count(),
                'totalMediumPriorityTasks' => Task::whereIn('id', $allTaskIds)->where('priority', 2)->count(),
                'totalHighPriorityTasks' => Task::whereIn('id', $allTaskIds)->where('priority', 1)->count(),
                'totalPerformanceOnTrack' => Task::whereIn('id', $allTaskIds)->where('completed', 0)->count(),
                'totalPerformanceBeforeTime' => Task::whereIn('id', $allTaskIds)->where('completed', 1)->whereColumn('due_date', '>', 'completed_date')->count(),
                'totalPerformanceDelayed' => Task::whereIn('id', $allTaskIds)->where('completed', 1)->whereColumn('due_date', '<', 'completed_date')->count(),
                'totalIncompletedProcessTasks' => Task::whereIn('id', $allTaskIds)->where('completed', 1)->count(),
                'totalIncompletedPendingTasks' => Task::whereIn('id', $allTaskIds)->where('completed', 0)->count(),
            ];
        ...
        public function userAllPermission(Request $request){
            $userId = Auth::user()->id;
            $userData = User::where('id',$userId)->first();
            if($userData){
                // Projects
                $projectUserModel = UserPermission::where('user_role_id',$userData->assignRole)->where('user_model_id',1)->first();
                $projectPermission['add'] = 1;
                $projectPermission['edit'] = 1;
                $projectPermission['delete'] = 1;
                if($projectUserModel){
                    $projectPermission['add'] = $projectUserModel->add;
                    $projectPermission['edit'] = $projectUserModel->edit;
                    $projectPermission['delete'] = $projectUserModel->delete;
                }

                // Tasks
                $taskUserModel = UserPermission::where('user_role_id',$userData->assignRole)->where('user_model_id',2)->first();
                $taskPermission['add'] = 1;
                $taskPermission['edit'] = 1;
                $taskPermission['delete'] = 1;
                if($taskUserModel){
                    $taskPermission['add'] = $taskUserModel->add;
                    $taskPermission['edit'] = $taskUserModel->edit;
                    $taskPermission['delete'] = $taskUserModel->delete;
                }

                // Discussion
                $discussionUserModel = UserPermission::where('user_role_id',$userData->assignRole)->where('user_model_id',3)->first();
                $discussionPermission['add'] = 1;
                $discussionPermission['edit'] = 1;
                $discussionPermission['delete'] = 1;
                if($discussionUserModel){
                    $discussionPermission['add'] = $discussionUserModel->add;
                    $discussionPermission['edit'] = $discussionUserModel->edit;
                    $discussionPermission['delete'] = $discussionUserModel->delete;
                }

                // Note
                $noteUserModel = UserPermission::where('user_role_id',$userData->assignRole)->where('user_model_id',4)->first();
                $notePermission['add'] = 1;
                $notePermission['edit'] = 1;
                $notePermission['delete'] = 1;
                if($noteUserModel){
                    $notePermission['add'] = $noteUserModel->add;
                    $notePermission['edit'] = $noteUserModel->edit;
                    $notePermission['delete'] = $noteUserModel->delete;
                }

                // Organization Management
                $organizationUserModel = UserPermission::where('user_role_id',$userData->assignRole)->where('user_model_id',5)->first();
                $organizationPermission['add'] = 1;
                $organizationPermission['edit'] = 1;
                $organizationPermission['delete'] = 1;
                if($organizationUserModel){
                    $organizationPermission['add'] = $organizationUserModel->add;
                    $organizationPermission['edit'] = $organizationUserModel->edit;
                    $organizationPermission['delete'] = $organizationUserModel->delete;
                }
            }
            $response['projectPermission'] = $projectPermission;
            $response['taskPermission'] = $taskPermission;
            $response['discussionPermission'] = $discussionPermission;
            $response['notePermission'] = $notePermission;
            $response['organizationPermission'] = $organizationPermission;
            
            if($response){
                return response()->json([
                    'status' => true,
                    'message' => 'User Permission list successfully',
                    'data' => $response
                ]);
            } else {
                return response()->json([
                    'status' => false,
                    'message' => 'User Permission data not found',
                    'data' => []
                ]);
            }
        }
        ...
        public function userTypeList(Request $request){
            ...
            if($getData){
                return response()->json([
                    'status' => true,
                    'message' => 'Users Type list successfully',
                    'data' => $getData
                ]);
            } else {
                return response()->json([
