                            ...
                            <form method="post" enctype="multipart/form-data" action="{{ route('project.update',$project->id) }}">
                                ...
                                <div class="row">
                                    <div class="col-sm-12 form-group">
                                        ...
                                        <input type="text" class="form-control" name="name" value="{{ $project->name }}" require placeholder="Enter Project Name">
                                        @if ($errors->has('name'))
                                    ...
                                    <div class="col-sm-12 form-group">
                                        ...
                                        <textarea id="description" name="description" class="form-control" placeholder="Enter Description">{{ $project->description }}</textarea>
                                        @if ($errors->has('description'))
                                            <span class="text-danger">{{ $errors->first('description') }}</span>
                                        @endif
                                    </div>
                                    @php 
                                        $start_end_date = date("m-d-Y", strtotime($project->start_date)) . ' - ' . date("m-d-Y", strtotime($project->end_date));
                                    ...
                                    <div class="col-sm-12 form-group">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text" name="start_end_date" value="{{ $start_end_date }}" class="form-control float-right" id="reservation">
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="select2" name="client_id" id="client_id" data-placeholder="Select a Client" style="width: 100%;">
                                                @foreach($clientUser as $val)
                                                    <option value="{{ $val->id }}" @if($project->client_id == $val->id) selected="selected" @endif>{{ $val->name }}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            ...
                                            <select class="select2" multiple="multiple" name="members_id[]" id="members_id" data-placeholder="Select a Memebers" style="width: 100%;">
                                                @php $members_ids = explode(",", $project->members_id); @endphp
                                    ...
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            ...
                                            <select class="select2" multiple="multiple" name="manager_id[]" id="manager_id" data-placeholder="Select a Project Manager" style="width: 100%;">
                                                @php $manager_ids = explode(",", $project->manager_id); @endphp
                                                @foreach($teamLeader as $val)
                                                    <option value="{{ $val->id }}" {{ in_array($val->id, $manager_ids) ? 'selected="selected"' : '' }}>{{ $val->name }}</option>
                                                @endforeach
                                            </select>
