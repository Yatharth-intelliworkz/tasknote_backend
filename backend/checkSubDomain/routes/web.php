...
Route::group(['domain' => '{account}.tasknote.in'], function () {
    Route::middleware('set.db.connection')->group(function () {
        Route::get('/dbCheck/{code}', [App\Http\Controllers\Controller::class, 'dbCheck'])->name('dbCheck');
        // You can add more routes that require dynamic database connection here
    });
    // Route::get('/', function ($account) {
    //     // This will return the account name based on the subdomain accessed.
    //     return "Welcome to the account: " . $account;
    // });

    // // Example of another route under the same subdomain
